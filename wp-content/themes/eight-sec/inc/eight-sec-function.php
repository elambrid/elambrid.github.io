<?php
    ...
    function eight_sec_no_sidebar ($classes) {
        $sidebar_class = 'right-sidebar';
        if(is_page_template('tpl-home.php')):
            $classes[]= 'no-sidebar';
        endif;
        if( ( $sidebar_class == 'right-sidebar' ) && !is_active_sidebar( 'sidebar-1' ) ) {
            $sidebar_class = 'no-sidebar';
        }
        $classes[] = $sidebar_class;
        return $classes;
    }   
    add_filter( 'body_class', 'eight_sec_no_sidebar' );
    ...
    if (is_home() || is_front_page()) {
        ...
        if (is_category()) {
            ...
            if (get_post_type() != 'post') {
                ...
                if ($showCurrent == 0)
                $cats = preg_replace_callback("|^(.+)$sep$|", function ($matches) {return $matches['1']; }, $cats);
                echo $cats;
                if ($showCurrent == 1)
