...
// constructor
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['quill_latest_news']) )
			delete_option('quill_latest_news');		  
		  
		return $instance;
	}
	
	function flush_widget_cache() {
		wp_cache_delete('quill_latest_news', 'widget');
	}
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'quill_latest_news', 'widget' );
		}
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Latest news', 'quill' );
...
?>
			...
			<div class="container">
				...
				<div class="clearfix">
					<?php while ( $r->have_posts() ) : $r->the_post(); ?>
						<div class="blog-post col-md-4 col-sm-6 col-xs-6 wow fadeInUp">
				...
				<a href="<?php echo esc_url( get_permalink( get_option( 'page_for_posts' )) ); ?>" class="read-more buttons wow zoomInUp"><?php echo __('See all our news', 'quill'); ?></a>
			</div>
		<?php if ($image_uri != '') : ?>
			<style type="text/css">
				.latest_news-area {
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'quill_latest_news', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
