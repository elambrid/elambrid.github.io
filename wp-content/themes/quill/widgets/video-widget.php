...
// constructor
	...
	<p><label for="<?php echo $this->get_field_id( 'url' ); ?>"><?php _e( 'Paste the URL of the video (only from a network that supports oEmbed, like Youtube, Vimeo etc.):', 'quill' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['quill_video_widget']) )
			delete_option('quill_video_widget');		  
		  
		return $instance;
	}
	
	function flush_widget_cache() {
		wp_cache_delete('quill_video_widget', 'widget');
	}
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'quill_video_widget', 'widget' );
		}
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Video', 'quill' );
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'quill_video_widget', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
