...
// constructor
	...
	<p><?php _e('In order to display this widget, you must first add some testimonials from the dashboard. Add as many as you want and the theme will automatically display them all.', 'quill'); ?></p>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['quill_testimonials']) )
			delete_option('quill_testimonials');		  
		  
		return $instance;
	}
	
	function flush_widget_cache() {
		wp_cache_delete('quill_testimonials', 'widget');
	}
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'quill_testimonials', 'widget' );
		}
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'What our clients say', 'quill' );
...
?>
			...
			<div class="container">	
				...
				<?php while ( $r->have_posts() ) : $r->the_post(); ?>
					...
					<li class="testimonial">
						<?php if ($photo != '') : ?>
							<img class="client-photo col-md-4 col-sm-4" src="<?php echo esc_url($photo); ?>" alt="<?php the_title(); ?>">
						<?php endif; ?>
						<div class="testimonial-body col-md-8 col-sm-8"><?php the_content(); ?></div>
						<h4 class="client-name col-md-8 col-sm-8"><?php the_title(); ?></h4>
						<?php if ($function != '') : ?>
							<span class="client-function col-md-8 col-sm-8"><?php echo esc_html($function); ?></span>
						<?php endif; ?>						
					</li>
				<?php endwhile; ?>
				</ul>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'quill_testimonials', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
