      ...
      function plugin_row_meta( $links, $file ){
		...
		function forms_list() {
			if ( isset( $_GET['action'], $_GET['form_id'] ) && $_GET['action'] == 'edit-form' ) {
				...
				if ( $data['form_row'] != null ) {
					...
					echo  __( 'No form found for this form id.Please go back and create a new form.', 'ultimate-form-builder-lite' );
				}
			} else {
		...
		function register_admin_assets() {
			...
			if ( isset( $_GET['page'] ) && in_array( $_GET['page'], $plugin_pages ) ) {
				...
				wp_enqueue_style( 'ufbl-font-awesome', UFBL_CSS_DIR.'/font-awesome.min.css', array(), UFBL_VERSION );
